wp.customize.controlConstructor['arrival-range'] = wp.customize.Control.extend({
	...
	ready: function() {
		...
		var control = this,
			...
			input_default,
			changeAction,
			arrival_range_input_number_timeout;

		// Update the text value
		jQuery( 'input[type=range]' ).on( 'mousedown', function() {

			range 			= jQuery( this );
			range_input 	= range.parent().children( '.arrival-range-input' );
		...
		function arrival_autocorrect_range_input_number( input_number, timeout ) {
			...
			clearTimeout( arrival_range_input_number_timeout );

			arrival_range_input_number_timeout = setTimeout( function() {
		...
		jQuery( 'input.arrival-range-input' ).on( 'change keyup', function() {

			arrival_autocorrect_range_input_number( jQuery( this ), 1000);

		} ).on( 'focusout', function() {

			arrival_autocorrect_range_input_number( jQuery( this ), 0);

		} );

		// Handle the reset button
		jQuery( '.arrival-reset-slider' ).on('click', function() {
